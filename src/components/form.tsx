...
import { Phone, Upload, X, Image as ImageIcon, ChevronDown } from "lucide-react";
    ...
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="flex">
        ...
        <Input
          ...
          className={`rounded-l-none ${errorMessage ? "border-destructive" : ""}`}
        />
      </div>
      {errorMessage && (
...
export function TextInput({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <Label htmlFor={title.toLowerCase().replace(/\s+/g, '-')} className="text-base font-medium">
        {title} {isRequired && <span className="text-destructive">*</span>}
      </Label>
      {description && (
        <p className="text-sm text-muted-foreground">{description}</p>
      ...
      <InputComponent
        ...
        {...(isParagraph && { className: `min-h-[100px] ${errorMessage ? "border-destructive" : ""}` })}
      />
      {errorMessage && (
        <p className="text-sm text-destructive">{errorMessage}</p>
      )}

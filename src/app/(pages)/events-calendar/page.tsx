...
import { Event, Preferences, Organization } from "./types/calendar";
...
export default async function Page() {
  async function fetchEvents() {
    try {
      ...
      const apiUrl = `${base_url}/api/events?startTime=${startTime}&endTime=${endTime}`;
  ...
  async function fetchPreferences() {
    try {
      const base_url = process.env.BASE_URL || 'http://localhost:3000';
      const apiUrl = `${base_url}/api/events/preferences`;
